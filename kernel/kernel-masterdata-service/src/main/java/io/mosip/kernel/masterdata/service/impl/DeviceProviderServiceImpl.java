...
public class DeviceProviderServiceImpl implements
	...
	public ResponseDto validateDeviceProviders(ValidateDeviceDto validateDeviceDto) {
		...
		checkMappingBetweenDigitalIdAndService(validateDeviceDto.getDigitalId().getDpId(),
				validateDeviceDto.getDigitalId());
		...
		try {
			...
			throw new MasterDataServiceException(
					DeviceProviderManagementErrorCode.NO_SUCH_FIELD_EXCEPTION.getErrorCode(),
					DeviceProviderManagementErrorCode.NO_SUCH_FIELD_EXCEPTION.getErrorMessage());
		}
	...
	private boolean checkMappingBetweenDigitalIdAndService(String providerId, DigitalIdDto digitalIdDto) {
		MOSIPDeviceService deviceService = null;
		try {
			deviceService = deviceServiceRepository
					.findByDeviceProviderIdAndRegDeviceTypeCodeAndRegDeviceSubCodeAndMakeAndModel(providerId,
							digitalIdDto.getType(), digitalIdDto.getSubType(), digitalIdDto.getMake(),
	...
	private void validateDigitalIdWithRegisteredDeviceHistory(RegisteredDeviceHistory registeredDevice,
			DigitalIdDto digitalIdDto) throws NoSuchFieldException, SecurityException {
		List<ServiceError> serviceErrors = new ArrayList<>();
		if (!registeredDevice.getMake().equals(digitalIdDto.getMake())) {
			ServiceError serviceError = new ServiceError();
			serviceError
